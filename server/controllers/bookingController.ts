...
export const createBooking = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
        ...
        if (!restaurantId || !date || !time || !guests) {
            res.status(400).json({ message: "Please provide all required reservation" });
            return;
        }   
        ...
        if (requestedGuests > availableSeats) {
            res.status(400).json({ message: `Unable to reserve. Only ${availableSeats} seats are available for the selected time slot` });
            return;
        }
...
export const getMyBookings = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
        ...
        ("restaurant", "name location image address slug").sort({ date: -1, time: -1});
        res.json(bookings);
    } catch (error: any) {
        console.error(error);
        res.status(400).json({ message: error.message });
