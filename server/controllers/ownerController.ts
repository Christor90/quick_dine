...
export const  createOwnerRestaurant= async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
        ...
        if(existing) {
            res.json(400).json({message: "You already have a restaurant registered"});
            return;
        }
        ...
        if(slugExists){
            res.status(400).json({ message: "A restaurant with this name already exist"});
            return;
        }
...
export const updateBookingStatus = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
        ...
        // verify booking belongs to the owner's retaurant
